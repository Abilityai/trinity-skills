...
argument-hint: "[--check]"
deprecated: true
superseded-by: "orchestrate — Abilityai/tandem-skill-library (private; not reachable from this public catalog)"
metadata:
  version: "1.32"
  created: 2026-07-01
  author: Ability.ai
  changelog:
    - "1.32: RETIRED from the public library (2026-09-24) — its 7 runtime skills (discover-agents, orchestrate, compose-system, profile-fleet, fleet-reconcile, sync-fleet-to-head, reconcile-skill-map) now live in Abilityai/tandem-skill-library, a private second skills-library source (operator ruling 2026-09-23/24: a deployed agent's ongoing skill diet comes from a library, never a marketplace plugin install). That successor is Ability.ai-internal and not reachable from this public catalog — an external user who needs fleet orchestration still has the abilities marketplace installer (Abilityai/abilities, unaffected by this retirement) as a local, one-time scaffold. This entry is no longer mirrored from abilities and will be removed at the next minor tag."
