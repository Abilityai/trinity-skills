...
metadata:
  ...
  author: orchestrator
  changelog:
    - "1.16: Platform-truth refresh (Trinity dev 9ac2ceae, 0.9.5-rc2): fan_out is N tasks to ONE agent (1–50) — many agents = per-agent chat_with_agent(parallel=true, async=true); a fan_out_timeout receipt is polled via get_fan_out_result (allowed-tools + error-table row, #2670); rooms are OSS core since ent#443 with defaults 200 messages / 168h (#2620); deploy_local_agent archives must embed .trinity-manifest.json (#2060); get_agent_skills lists library-assigned skills only — repo-native playbooks are read from the workspace; ent#411 shipped (trinity plugin pre-installed, no CLI bootstrap); a failed sync task releases its idempotency claim (#2661)"
    - "1.15: Spec-less catalog members: an ephemeral rollout of a repo with no template.yaml still works (deploy_system tolerates it) but the member arrives with no declared resources/plugins/schedules — say so in the plan, pass resources in the minimal manifest, and if the work actually needs the member DURABLY Trinity-compatible that is not an ephemeral: run the ladder create-as-is → dispatch `/trinity:onboard in-place` (trinity plugin v6.0) → `/discover-agents` instead (see /compose-system Step 3). Playbook-call form only; the ent#411 bootstrap caveat named once"
...
| **Fan-out** | same task over many inputs, all on **one** agent (1–50 tasks) | `fan_out` — for many *agents*, per-agent `chat_with_agent(parallel=true, async=true)` |
...
Rooms are **bounded**: they auto-close on the message budget (default 200), the cost budget, or `ttl_hours` (default 168; `0` = never). A room is therefore not a place to park long work — dispatch that with the fire-and-park contract instead. Rooms are OSS core since ent#443; `{"error": "shared_sessions_not_enabled"}` now only means an older build or an unmounted route — treat it as "fall back to a chain".
...
- **Resolve the work to a playbook, not a brief** (fleet convention `protocols/playbook-call.md`): read the target's *live* catalog — `mcp__trinity__get_agent_skills` lists **library-assigned** skills only; for repo-native playbooks read its `.claude/skills/` from the map's source (or ask it `/help`) — never the map or a fleet-profile snapshot (playbooks change faster than profiles) — and pick the playbook that owns this step. The dispatched message is then **one line**: `/<playbook> [declared args] --run <task_id>` — no restated policy, no preamble; the playbook is the contract. If no playbook fits, dispatch a prose brief **and record `freeform: true`** on the ledger entry — that is a fleet gap to report (the owner should grow a playbook), not the normal path.
...
- **catalog-only (`deployed: false`)** → it must be rolled out. Confirm the ephemeral plan **once, up front**: list which agents will be created and that they'll be **torn down when the task completes**. On approval:
  - GitHub source (**the normal case** — a catalog member should be a repo):
    ...
    - **Spec-less member** (`spec_found: []` in the map — no `template.yaml`): the deploy still works, but it arrives with **no** declared resources/plugins/schedules — pass `resources` in the minimal manifest and say so in the plan. If the task needs the member to *stay* Trinity-compatible, that is not an ephemeral: create it as-is, dispatch **`/trinity:onboard in-place`** to it (the `trinity` plugin is pre-installed in the base image since ent#411 — no CLI bootstrap; opt-out is `TRINITY_PLATFORM_PLUGINS=0`), then `/discover-agents` — see `/compose-system` Step 3.
  ...
  - Local-only source (**fallback**) → `mcp__trinity__deploy_local_agent` with an archive that embeds `.trinity-manifest.json` (#2060 — the tool description carries the one-liner; a missing/drifting manifest is refused 400 `MANIFEST_REQUIRED`/`MANIFEST_DRIFT`; ~100–200 KB base64 ceiling — bigger agents via `trinity deploy .`). It works, but the rollout isn't reproducible — note it in the report and flag the member for a repo (`gh repo create <name> --private --source=. --push`, then update `source:` in the map) so the next rollout takes the repo path.
...
- **Fan-out, quick:** `fan_out(<deployed_name>, tasks[])` fans N tasks (1–50) to **one** agent → collect results; a `fan_out_timeout` receipt means the batch is still running — record `fan_out_id`, poll `get_fan_out_result(agent_name, fan_out_id)`, never re-send. Many *agents* = per-agent `chat_with_agent(parallel=true, async=true)` dispatches (also the choice when legs run long), with one ledger entry per leg.
...
| `fan_out` returns `fan_out_timeout` | Batch still running — record `fan_out_id`, poll `get_fan_out_result(agent_name, fan_out_id)`; `partial` is a normal outcome; never re-send |
